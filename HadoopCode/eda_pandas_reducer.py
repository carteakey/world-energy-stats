...
for line in sys.stdin: # for each line in ouput generated by mapper
    ...
    col_name, null_c, min, max, sum, count = line.split('\t') # sperate values by \t delimeter
    ...
    if col_name in dict: # If column has already entry in dictionary
        ...
        if col_name in ['iso_code','country']: 
            ...
            dict[col_name][min] = min_val if min_val < dict[col_name][min] else dict[col_name][min]
            # If current maximum is more then, present - replace max val
            dict[col_name][max] = max_val if max_val > dict[col_name][max] else dict[col_name][max]
        ...
        dict[col_name] = { null : n_count, min: min_val, max: max_val, sum: sum_val, t_count: count_val }
...
for column, val in dict.items():
    ...
    if col_name in ['iso_code','country']: 
        ...
        print(f"{column}\tNull_Count:{val['null_count']}\tMin:{val['min']}\tMax:{val['max']}\tMean:{mean_val}")
